    ...
    container.innerHTML = allNotifications.map(item => {
        const badgeHtml = `
            <div class="history-badges">
                ${item.pinned ? '<span class="history-badge fixed"><span class="material-symbols-rounded">push_pin</span>蝗ｺ螳・/span>' : ''}
                ${item.type === 'emergency' ? '<span class="history-badge emergency"><span class="material-symbols-rounded">error</span>邱頑･</span>' : ''}
                ${item.type === 'urgent' ? '<span class="history-badge urgent"><span class="material-symbols-rounded">priority_high</span>驥崎ｦ・/span>' : ''}
            </div>
        `;
